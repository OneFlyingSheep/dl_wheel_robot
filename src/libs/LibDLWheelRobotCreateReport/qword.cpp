...
QWord::QWord(QObject *parent) : QObject(parent)
{
...
bool QWord::createNewWord()		//创建一个新的word
{
	...
	QString defaultFileName = tr("demo");
	m_saveName = QFileDialog::getSaveFileName(0, QString("日志信息"), defaultFileName, tr("*.doc"));
...
bool QWord::Open(QString sFile)
{
	...
	m_documents->dynamicCall("Add (Qstring)", m_saveName);
	m_document = m_word->querySubObject("ActiveDocument");//获取当前激活的文档
...
// 	range->querySubObject("InlineShapes")->dynamicCall("AddPicture(const QString&)", picPath);
	...
	if (table)
	{
		QAxObject* range = table->querySubObject("Cell(int, int)", row, column)->querySubObject("Range");
		range->querySubObject("InlineShapes")->dynamicCall("AddPicture(const QString&)", picPath);
		range->setProperty("Width", 100);
		range->setProperty("Height", 100);
	}
}
