#ifndef NSWITCH_CONTROL
#define NSWITCH_CONTROL
...
class NSwitchControl : public QWidget
{
	Q_OBJECT

public:
	explicit NSwitchControl(QWidget *parent = 0);
...
class NSwitchWidget : public QWidget
{
	Q_OBJECT

public:
	NSwitchWidget(bool isWheelRobot = false)
...
private:
	void initControl()
	{
		m_pSwitchControl = new NSwitchControl(this);
		m_pSwitchControl->setFixedSize(QSize(50, 20));
		// 设置状态、样式;
		m_pSwitchControl->setCheckedColor(QColor(0, 0, 0));
		...
		if (m_isWheelRobot)
		{
			m_switchNamelabel->setStyleSheet("color:rgb(0, 0, 0);font-size:14px;");
...
private:
	NSwitchControl * m_pSwitchControl;
