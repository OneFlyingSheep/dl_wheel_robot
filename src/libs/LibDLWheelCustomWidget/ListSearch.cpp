...
void ListSearch::SearchItem(QListWidget *pListWgt, const QString &strKeyWord)
{
    m_pCurListWgt = pListWgt;
    if(NULL == m_pCurListWgt)
    {
        return;
    }
...
void ListSearch::SetIsAllItemShow(bool bIsShow)
{
	if (NULL != m_pCurListWgt)
	{
		for (int iRow = 0; iRow < m_pCurListWgt->count(); ++iRow)
		{
			QListWidgetItem *pItem = m_pCurListWgt->item(iRow);
			if (NULL != pItem)
			{
				pItem->setHidden(!bIsShow);
			}
		}
	}
}

void ListSearch::FuzzySearch(QString &strKeyWord)
{//模糊搜索
	SetIsAllItemShow(false);					//隐藏所有节点
	int iRootCount = m_pCurListWgt->count();
	for (int iRow = 0; iRow < iRootCount; ++iRow)
	{
		QListWidgetItem *pCurItem = m_pCurListWgt->item(iRow);
		if (pCurItem == NULL)
		{
			continue;
		}
		QString strCurRootItem = pCurItem->text();
...
void ListSearch::PreciseSearch(const QString &strKeyWord)
{//精确搜索
	SetIsAllItemShow(false);					//隐藏所有节点
	int iRootCount = m_pCurListWgt->count();
	for (int iRow = 0; iRow < iRootCount; ++iRow)
...
void ListSearch::GetPinYin(const QString &strHanZi, QString &strInitial, QString &strQuanPin)
{
	//保存拼音的ASCII值
...
void ListSearch::GetWString(const std::string &strIn, std::wstring &wStrOut)
{
...
QListWidget * ListSearch::m_pCurListWgt = NULL;
