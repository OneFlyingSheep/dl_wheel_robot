...
CustomCalendarInfo *CustomCalendarInfo::self = 0;
CustomCalendarInfo *CustomCalendarInfo::Instance()
{
    if (!self) {
        QMutex mutex;
        QMutexLocker locker(&mutex);
        if (!self) {
            self = new CustomCalendarInfo;
        }
    }
...
CustomCalendarInfo::CustomCalendarInfo(QObject *parent) : QObject(parent)
{
    //农历查表
...
//计算是否闰年
bool CustomCalendarInfo::isLoopYear(int year)
{
    return (((0 == (year % 4)) && (0 != (year % 100))) || (0 == (year % 400)));
}

//计算指定年月该月共多少天
int CustomCalendarInfo::getMonthDays(int year, int month)
{
    int countDay = 0;
...
//计算指定年月对应到该月共多少天
int CustomCalendarInfo::getTotalMonthDays(int year, int month)
{
    int countDay = 0;
    int loopDay = isLoopYear(year) ? 1 : 0;
...
//计算指定年月对应星期几
int CustomCalendarInfo::getFirstDayOfWeek(int year, int month)
{
    int week = 0;
...
//计算国际节日
QString CustomCalendarInfo::getHoliday(int month, int day)
{
...
//计算二十四节气
QString CustomCalendarInfo::getSolarTerms(int year, int month, int day)
{
...
//计算农历节日(必须传入农历年份月份)
QString CustomCalendarInfo::getLunarFestival(int month, int day)
{
    int temp = (month << 8) | day;
    QString strFestival;
...
//计算农历年 天干+地支+生肖
QString CustomCalendarInfo::getLunarYear(int year)
{
    QString strYear;
    if (year > 1924) {
        int temp = year - 1924;
...
void CustomCalendarInfo::getCustomCalendarInfo(int year, int month, int day,
        QString &strHoliday,
        QString &strSolarTerms,
...
QString CustomCalendarInfo::getLunarInfo(int year, int month, int day, bool yearInfo, bool monthInfo, bool dayInfo)
{
    QString strHoliday, strSolarTerms, strLunarFestival, strLunarYear, strLunarMonth, strLunarDay;

    CustomCalendarInfo::Instance()->getCustomCalendarInfo(year, month, day,
...
QString CustomCalendarInfo::getLunarYearMonthDay(int year, int month, int day)
{
    return getLunarInfo(year, month, day, true, true, true);
}

QString CustomCalendarInfo::getLunarMonthDay(int year, int month, int day)
{
    return getLunarInfo(year, month, day, false, true, true);
}

QString CustomCalendarInfo::getLunarDay(int year, int month, int day)
{
