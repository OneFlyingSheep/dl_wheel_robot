...
BOOL ParseUrl::Ping(const char *szDestIP, PingReply *pPingReply, DWORD dwTimeout)
{
	if (NULL != szDestIP)
	{
...
bool ParseUrl::getIpConnectState(QString strIp, int timeout)
{
    QString cmdstr = QString("ping %1 -n 1 -w %2")
        .arg(strIp).arg(timeout);
...
bool ParseUrl::getIpAndPortConnectState(QString strIp, int port, int timeout)
{
    QTcpSocket tcpClient;
    tcpClient.abort();
    tcpClient.connectToHost(strIp, port);
    //100毫秒没有连接上则判断不在线
    bool isConnect = tcpClient.waitForConnected(timeout);
    tcpClient.close();
