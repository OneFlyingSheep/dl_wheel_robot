    ...
    , m_isStopImportExcelData(false)
{
	...
	QFile file("C:\\Users\\18516\\Desktop\\2.txt");
	...
	while (!in.atEnd()) {
		...
		WHEEL_ROBOT_DB.insertFeverType(line.split(",")[0]);
	}
...
void DLWheelStandardPointPosMaintain::initTableWidget()
{
    ...
    m_customTableWidget->setHorizontalHeaderLabels(QStringList() << "序号" << "设备类型" << "小类设备" << "点位名称" << "识别类型" << "表计类型" << "设备序列号" << "保存类型" << "操作");
...
void DLWheelStandardPointPosMaintain::exportTable()
{
    ...
    excelData.append(QStringList() << "设备类型" << "小类设备" << "点位名称" << "识别类型" << "表计类型" << "设备序列号" << "保存类型");
...
void DLWheelStandardPointPosMaintain::file_import_patrol_list( )
{
	...
	while (getline(file, file_string))
	{
		...
		data.m_device_type_name = QString().fromStdString(temp_string.substr(0, blank_index));
		temp_string = temp_string.substr(blank_index + 1);

		//sub_device_type_id
		blank_index = temp_string.find_first_of(',');
		sub_device_type_id = atoi(temp_string.substr(0, blank_index).c_str());
		data.m_sub_device_name = QString().fromStdString(temp_string.substr(0, blank_index));
		temp_string = temp_string.substr(blank_index + 1);

		//device_point_name
		blank_index = temp_string.find_first_of(',');
		data.m_device_point_name = QString().fromStdString(temp_string.substr(0, blank_index));
		temp_string = temp_string.substr(blank_index + 1);

		//recognition_type
		blank_index = temp_string.find_first_of(',');
		data.m_recognition_type_name = QString().fromStdString(temp_string.substr(0, blank_index));
		temp_string = temp_string.substr(blank_index + 1);

		//meter_type
		blank_index = temp_string.find_first_of(',');
		data.m_meter_type_name = QString().fromStdString(temp_string.substr(0, blank_index));
		temp_string = temp_string.substr(blank_index + 1);

		//device_sn 不需要处理
		blank_index = temp_string.find_first_of(',');
		data.m_fever_type_name = QString().fromStdString(temp_string.substr(0, blank_index));
		temp_string = temp_string.substr(blank_index + 1);

		//save_type
		blank_index = temp_string.find_first_of(',');
		data.m_save_type_name = QString().fromStdString(temp_string.substr(0, blank_index));
		temp_string = temp_string.substr(blank_index + 1);

		m_addPointPosWidget->insertData(data);
	}
